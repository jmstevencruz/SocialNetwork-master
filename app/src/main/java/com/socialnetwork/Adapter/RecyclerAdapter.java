...
import com.socialnetwork.Objects.SkillsObj;
...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private int TYPE_SKILLS = 2;
    private GroupObj group;
    private SkillsObj skills;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        }if(ResultObj.get(0) instanceof SkillsObj){
            View view = LayoutInflater.from(context).inflate(R.layout.item_list_group, parent, false);
            return new RecyclerSearchSkills(view);
        }else{
            return null;
        }
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int position) {
        ...
        if (ResultObj != null){
            ...
            if(ResultObj.get(position) instanceof SkillsObj){
                RecyclerSearchSkills holder = (RecyclerSearchSkills) viewHolder;
                holder.bind((SkillsObj) ResultObj.get(position),context,position,type,listenerUrl);
            }
        }
    }
    ...
    public static class RecyclerSearchSkills extends RecyclerView.ViewHolder {

        private TextView txtName;

        public RecyclerSearchSkills(final View parent) {
            super(parent);
            parent.setClickable(true);
            txtName =  parent.findViewById(R.id.id_group);
        }

        public void bind(final SkillsObj item, final Context context, final int position, int type, final OnClickListener listener) {
            txtName.setText(item.getName());
    ...
    public int getItemViewType(int position) {
        ...
        if(ResultObj != null && !ResultObj.isEmpty()){
            if(ResultObj.get(position) instanceof GroupObj){
                return TYPE_GROUP;
            }else if(ResultObj.get(position) instanceof SkillsObj){
                return TYPE_GROUP;
            }
        }else{
            return 0;
